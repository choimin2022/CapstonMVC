...
// displayMessage6 함수 수정
function displayMessage6(pages, index = 0) {
  // ...
  for (var pageIndex = index; pageIndex < pages.length; pageIndex++) {
  ...
  // display를 자바DAO DTO로 수정
  var _class = "yourmsg";
  var _val = pages[index].message;
  var buttonHtml = '';
  var sBtn = '';
  var imageHtml = '';
  ...
  for (var j = 0; j < pages.length; j++) {
    if (j == index) {
      imageHtml += '<img class="message-image active" src="' + pages[j].imageAddress + '" alt="image">';
      _val = pages[j].message;
    } else {
  ...
  // 페이지가 2개 이상일 때
  if (pages.length > 1) {
    // 현재 표시할 페이지의 인덱스와 동일한 li 요소에 checked 클래스 추가
    const li = document.querySelector('.disBtn li:nth-child(' + (index + 1) + ')');
  ...
  var _tar = $(".chat_wrap .inner")
    .append('<div class="item ' + _class + '"><img class="tiger_chat" alt="image" src="img/img_Main/character_main5.png"><div class="box"><div class="msg">'
      ...
      + '<br>'
      + sBtn
      + '<div class="buttonP disBtn"><ul>' + buttonHtml + '</ul></div>'	 //div클래스 수정
      + '</div>'
      + '<span class="time">'
      + currentTime()
      + '</span></div></div>');
...
// changeContent2 함수 수정
function changeContent2(index) {
  // ...
  $(".message-image").removeClass("active");
  $(".message-image").eq(index).addClass("active");
  $(".val2-style").html('<img class="message-image active" src="' + pages[index].imageAddress + '" alt="image">' + pages[index].message);
}
